...
const NouvelleAffectation = async (req, res) => {
  try {
    ...
    console.error(error);
    return res.status(500).send('Une erreur est survenue lors de la création de l\'affectation');
  }
};
 ...
 /*  try {
  ...
  try {
    ...
    try {
      ...
      console.error(error);
      return res.status(500).send('Une erreur est survenue lors de la récupération des détails des affectations');
    }
};
