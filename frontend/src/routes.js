...
const auth = useSelector(state => state.auth)
  ...
  const routes = useRoutes([
    {
      path: '/dashboard',
      element: <PrivateRouter user={user}> <DashboardLayout /></PrivateRouter>,
      children: [
        { element: 
          <PrivateRouter user={user}> 
        <Navigate to="/dashboard/app" />
          </PrivateRouter> , index: true },
        { path: 'app', element: 
        <PrivateRouter user={user}>
        <DashboardAppPage />
        </PrivateRouter> },
        ...
        <PrivateRouter user={user}>
          <EditUser /> 
        </PrivateRouter>}, // Ajoutez une nouvelle route pour EditUser
        { path: 'vehicule', element: 
        <PrivateRouter user={user}>
          <VéhiculePage/>
        </PrivateRouter>},  /* <ProductsPage /> */
        { path: 'collaborateur', element:
        <PrivateRouter user={user}>
          <Collaborateur/> 
        </PrivateRouter>   },/* <BlogPage /> */
        { path: 'addCollaborateur' , element: 
        <PrivateRouter user={user}>
          <AddCollaborateur/>
        </PrivateRouter>   },
        { path: 'collab/:id' , element: 
        <PrivateRouter user={user}>
          <EditCollab/>
        </PrivateRouter>  }, 
        { path: 'addvehicule' , element: 
        <PrivateRouter user={user}>
          <AddVehicule/>
        </PrivateRouter>   }, 
        { path: 'editvehicule/:id' , element: 
        <PrivateRouter user={user}>
          <EditVehicule/>
        </PrivateRouter>   }, 
        { path: 'affectation' , element: 
        <PrivateRouter user={user}>
          <AffectationPage/>
        </PrivateRouter>   }
      ],
