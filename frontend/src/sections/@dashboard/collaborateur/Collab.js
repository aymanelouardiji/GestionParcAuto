...
const OnDelete = (id)=>{
    if(window.confirm("are you sure to delete this user")){
     ...
     Axios.delete(`http://localhost:3001/api/collab/${id}`)
     .then(res=>{   
      console.log(res.data)
      setMessage(res.data.message)
      setShow(true)
...
// HHHHHHHHHHHHHHHHHHHHHHHH 
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - collab.length) : 0;

  const visibleRows = React.useMemo(
    () =>
      stableSort(collab, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
            ...
            <TableBody>
              {visibleRows.map((collab, index) => {
                ...
                return (
                  ...
                  <TableRow hover onClick={(event) => handleClick(event, collab._id)}
                  ...
                  >
                    ...
                    <TableCell component="th" _id={collab._id} scope="row" padding="none"  >{collab.Nom}</TableCell>
                    ...
                    <TableCell align="right" ><Button component={RouterLink} to={`http://localhost:3000/dashboard/collab/${collab._id}`} endIcon={<EditIcon />}>Modifier</Button> <Button  endIcon={<DeleteIcon />} onClick={() => OnDelete(collab._id) }>Supprimer</Button></TableCell>
...
export default function TableCollab() {
  ...
  const getCollabDetails = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/collab');
      if (!response.ok) {
        throw new Error(
      ...
      const CollabWithId = data.map((Collaborateur) => ({
        ...Collaborateur,
        id: Collaborateur._id, // Utilisez la valeur de _id comme id
        ...
        NumeroGsm: Collaborateur.NumeroGsm,
        NumeroGsmPersonnel: Collaborateur.NumeroGsmPersonnel,
        TelephoneFixe: Collaborateur.TelephoneFixe,
        Permis: Collaborateur.Permis,
        DateValiditePermis: formatDate(Collaborateur.DateValiditePermis),
        CIN: Collaborateur.CIN,
        DateValiditéCIN: formatDate(Collaborateur.DateValiditéCIN),
        NumeroPassport: Collaborateur.NumeroPassport,
        DateValiditéPassport: formatDate(Collaborateur.DateValiditéPassport),
        
      }));
      setCollabDetails(CollabWithId);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const columns = [
   ...
   
    {
      ...
      renderCell: (params) => (
        ...
        to={`http://localhost:3000/dashboard/collab/${params.row.id}`}
      >
        <EditIcon />
      </IconButton>
          <IconButton
