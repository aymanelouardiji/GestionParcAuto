...
const [Vignete , setVignete]=useState(null)
  ...
  const onSubmitHandler = (e) => {
    ...
    axios.put(`http://localhost:3001/api/vehicule/${id}`, updatedVehicule)
      .then((res) => {
  ...
  const handleDate1Change = (date) => {
    ...
    setEcheance_Aut_Circulation(formattedDate);
  };

  const handleDate2Change = (date) => {
    const formattedDate = (date).format('MM/DD/YYYY');
    setEchaence_Visite_Tech(formattedDate);
  };

  const handleDate3Change = (date) => {
    const formattedDate = (date).format('MM/DD/YYYY');
    setAssurance_Contrat_Cours(formattedDate);
  };
  const handleDate4Change = (date) => {
    const formattedDate = (date).format('MM/DD/YYYY');
    setVignete(formattedDate);
  };
